// Build script for FiiO Control Portable 32-bit (ia32)
...
const outDir = path.join(distDir, 'FiiO-Control-ia32');
const outZip = path.join(distDir, 'FiiO-Control-Portable-v1.0.0-win-ia32.zip');

function buildIa32() {
  console.log('=====================================================');
  console.log('[Build] Building FiiO Control Portable Windows 32-bit (ia32)');
  console.log('=====================================================');

  fs.mkdirSync(distDir, { recursive: true });

  // 1. Locate or download electron ia32 zip
  ...
  if (fs.existsSync(cacheDir)) {
    ...
    for (const f of files) {
      if (typeof f === 'string' && f.includes('win32-ia32.zip')) {
  ...
  if (!electronZip || !fs.existsSync(electronZip)) {
    electronZip = path.join(distDir, 'electron-win32-ia32.zip');
    if (!fs.existsSync(electronZip)) {
      console.log('[Download] Downloading Electron ia32 runtime...');
      execSync(`curl.exe -L -s "https://github.com/electron/electron/releases/download/v31.4.0/electron-v31.4.0-win32-ia32.zip" -o "${electronZip}"`, { stdio: 'inherit' });
    }
  }

  // 2. Extract runtime
  console.log('[Extract] Extracting Electron ia32 runtime...');
  ...
  const newExe = path.join(outDir, 'FiiO-Control-ia32.exe');
  ...
  // Clean temp zip if downloaded
  const tempZip = path.join(distDir, 'electron-win32-ia32.zip');
  if (fs.existsSync(tempZip)) fs.unlinkSync(tempZip);

  console.log('[Done] FiiO Control Portable ia32 build completed successfully.');
}
